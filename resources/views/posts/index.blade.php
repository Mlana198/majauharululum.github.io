        ...
        <article class="bg-white rounded-lg shadow hover:shadow-lg transition">
          ...
          <div class="p-4">
            ...
            <div class="text-base text-gray-500 ">
              <a class="hover:underline" href="/authors/{{ $post->author->id }}">{{ $post->author->name }}</a> | {{ \Carbon\Carbon::parse($post['created_at'])->format('d M Y') }}
            </div>
            <p class="text-sm text-gray-600 mt-2">
              {{ Str::limit($post['body'], 100)}}
            </p>
            <a href="/posts/{{ $post['slug'] }}" class="inline-block mt-3 text-blue-600 hover:text-blue-800 font-medium">Baca selengkapnya →</a>
          </div>
        </article>
