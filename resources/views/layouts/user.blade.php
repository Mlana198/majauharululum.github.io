...
<div class="lg:hidden fixed top-0 inset-x-0 z-40 flex items-center justify-between
    ...
    <span class="font-semibold">User Panel</span>
...
<aside id="sidebar"
    ...
    <nav class="p-4 flex flex-col gap-1 text-sm">
        ...
        <a href="{{ route('admin.dashboard') }}"
           ...
           {{ request()->routeIs('user.dashboard') ? 'bg-blue-600' : 'hover:bg-gray-700' }}">
            Dashboard
        </a>

        {{-- Formulir PPDB selalu ada --}}
        <a href="{{ url('/formulir') }}"
        class="px-4 py-2 rounded
        {{ request()->is('formulir') ? 'bg-blue-600' : 'hover:bg-gray-700' }}">
            Formulir PPDB
        </a>

        {{-- HANYA tampil jika calonSiswa ada --}}
        @if(isset($calonSiswa))
            <a href="{{ route('formulir.ortu', $calonSiswa->id) }}"
            class="px-4 py-2 rounded
            {{ request()->routeIs('formulir.ortu*') ? 'bg-blue-600' : 'hover:bg-gray-700' }}">
                Formulir Orang Tua
            </a>

            <a href="{{ route('upload.form', $calonSiswa->id) }}"
            class="px-4 py-2 rounded
            {{ request()->routeIs('upload.*') ? 'bg-blue-600' : 'hover:bg-gray-700' }}">
                Upload Berkas
            </a>
        @endif
